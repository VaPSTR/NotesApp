...
class MainActivityFixed : AppCompatActivity() {
    ...
    private fun saveNote(content: String) {
        ...
        lifecycleScope.launch {
            ...
            val fileResult = FileManager.saveNote(this@MainActivityFixed, folderName, fileName, content)
            ...
            if (fileResult.isSuccess) {
                ...
                if (settings != null && 
                    ...
                    Toast.makeText(this@MainActivityFixed, "Заметка сохранена в Смартфоне", Toast.LENGTH_SHORT).show()
                    showMainMenu()
                }
            } else {
                Toast.makeText(this@MainActivityFixed, "Ошибка сохранения", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun startCopyToServer() {
        lifecycleScope.launch {
            ...
            if (settings == null || settings.serverIp.isBlank()) {
                Toast.makeText(this@MainActivityFixed, "Настройки сервера не заданы", Toast.LENGTH_SHORT).show()
                return@launch
            }
            ...
            if (notesToCopy.isEmpty()) {
                Toast.makeText(this@MainActivityFixed, "Нет заметок для копирования", Toast.LENGTH_SHORT).show()
                return@launch
            }
    ...
    private fun saveSettings() {
        ...
        lifecycleScope.launch {
            ...
            db.settingsDao().saveSettings(settings)
            Toast.makeText(this@MainActivityFixed, "Настройки сохранены", Toast.LENGTH_SHORT).show()
            showMainMenu()
        }
    }
